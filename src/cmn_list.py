...
class CMNLister:
    ...
    def show_xp_nodes(self, xp):
        ...
        for p in range(0, xp.n_device_ports()):
            ...
            connected_device_info = xp.port_object(p).connect_info
            connected_device_type = BITS(connected_device_info, 0, 5)
            if not cmn_port_device_type_has_properties(connected_device_type, self.port_props):
                continue
            print("      P%u:" % p, end="")
            ...
            for n in xp.port_nodes(p):
                ...
                if self.node_match is not None and not self.node_match.match_node(n):
                    if self.verbose:
                        print("        %s: skipped as does not match %s" % (n, self.node_match))
                    continue
                self.show_node(n)
    ...
    def show_node_event_sel(self, n):
        """
    ...
    def show_home_node_sam(self, n):
        ...
        print("          HN SAM control: 0x%x" % hn_sam_ctl)
        # Show range-based mapping first, as it takes priority
        for reg in hn_sam_regions(n):
            print("            region %u: %s" % (reg.index, reg.range_str()), end="")
        ...
        if sns:
            ...
            print("            direct-mapped: SN:0x%x" % BITS(hn_sam_ctl, 0, 11))
    ...
    def show_rn_sam(self, n):
        ...
        if BIT(gic, 0):
            print("        GIC: 0x%x" % gic)
        print("          Non-hashed memory regions:")
        for reg in rn_sam_nonhash_regions(n):
            print("            NHMR %3u: %s" % (reg.index, reg.range_str()), end="")
            print(" 0x%x %s" % (reg.nodeid, reg.target_type_str), end="")
            print()
        print("          Hashed memory regions:")
        for reg in rn_sam_hashed_regions(n):
            i = reg.index
            print("            HMR  %3u: %s" % (reg.index, reg.range_str()), end="")
            if reg.hn_count > 0:
            ...
            if reg.hashed and reg.nodeids is not None:
                print("              HNs: %s" % (','.join([("0x%x" % hn) for hn in reg.nodeids])))
        #hash_addr_mask = n.read64(0xF18)
        #print("          Hash mask: 0x%016x" % hash_addr_mask)
        if not n.C.part_ge_700():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--register-slices", action="store_true", help="show device/mesh register slices")
